...
4. Frosted Settings visual check: the window must show the frosted `popover` material as its real background (not transparent), identical in character to the menu-bar popover, in light and dark appearance; traffic lights do not collide with the tab row.
5. Settings UI check: the four tabs are independent capsule buttons with no enclosing border or divider lines; a stable gap separates the tab row from the content cards; cards render one layer above the glass (more solid fill, fine border, soft shadow); switching tabs preserves each page's scroll state; Tab focus + Space activation works on the tab buttons. Verify with Reduce Transparency on (cards near-opaque) and off.
