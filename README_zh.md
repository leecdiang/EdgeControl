...
EdgeControl 是 ad-hoc 签名且未公证，Gatekeeper 会隔离首次启动：可用 `brew install --cask --no-quarantine leecdiang/edgecontrol/edgecontrol` 安装，或首次运行时右键 > 打开。

**手动安装：** 从 [Releases 页面](https://github.com/leecdiang/EdgeControl/releases) 下载 `EdgeControl-<版本>-macOS.dmg`，打开后把应用拖入 Applications。
...
1.5.1 保留原生菜单，并提供过零、强档脉冲生命周期与多显示器 DDC 可靠性修复。1.6.0 在其基础上新增自定义磨砂菜单、分组式设置及三档 HUD 配色。1.6.1 修复实验性 DDC 应答字段偏移、补完整 ad-hoc bundle 签名并同步文档。1.7.0 让设置窗口与菜单栏弹窗一致地磨砂玻璃化、略微放宽边缘入口条、新增莫兰迪 HUD 配色，并发布 Homebrew cask。1.7.1 修复磨砂窗口渲染、再次放宽入口条、提亮莫兰迪音量色，并把设置标签改为无边框行 + 分层磨砂卡片。当前证据边界见 [BUILD_REPORT.md](BUILD_REPORT.md)。

手势识别、打字保护、数值映射、亮度路由、触控板选择策略、档位、触觉反馈与设置层由 68 个单元测试方法覆盖。依赖未公开 macOS ABI 的代码全部通过 `dlopen`/`dlsym` 动态加载，符号缺失时优雅降级（该功能不可用，应用照常运行），而不是启动崩溃。
...
- 单次手势锁定亮度后端：仅在明确开启且真实可用时考虑 External DDC；内屏瞬时枚举失败会重试；多显示器写入复用初始读取成功的连接
- 三档触觉反馈：轻档使用更稀疏的轻震，标准档保留原有 2% 手感，强档使用更有力的公开 AppKit 系统反馈；结束/复位时取消尚未触发的第二次脉冲
...
- System / Classic / Aurora / Morandi 四档 HUD 配色：主要改变进度填充条，玻璃与文字继续保持中性并适配深浅色
- 自定义轻量菜单栏弹窗：实时触控状态、左右边缘卡片及触觉/HUD 快捷控制
- 磨砂玻璃设置窗口：与菜单栏弹窗相同的整窗材质
- 分组式四页设置界面：Controls / Feedback / Devices / About，尺寸稳定
- Homebrew cask 安装（`brew install --cask leecdiang/edgecontrol/edgecontrol`）
...
调节速度与误触保护完全分开：速度只改变激活后的数值增益；误触保护同时选择打字窗口和物理边缘出生范围。强保护为 600ms、左右 1.0%/1.8%，标准为 350ms、1.2%/2.2%，轻保护为 200ms、1.5%/2.6%。左右不对称范围保留了真机测量差异。
...
| 参数 | 值 | 说明 |
| --- | ---: | --- |
| 入口条（左缘） | 1.2% | 出生点必须在此；内部出生永久拒绝 |
| 入口条（右缘） | 2.2% | 本机右缘出生点实测 x≈0.985–1.0 |
...
状态机：`idle → entryCandidate → entryConfirmed → active`。内部出生、最近打字、错误初始方向、超时、身份变化、离开走廊、多指都会产生当前生命周期的终态拒绝；激活后的过零保护固定使用触发激活的方向，缓慢反向无法借基线死区绕过取消；打字拒绝与 `multiTouchRejected` 都只在空帧后复位。
