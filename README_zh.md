...
1.5.1 保留原生菜单，并提供过零、强档脉冲生命周期与多显示器 DDC 可靠性修复。1.6.0 在其基础上新增自定义磨砂菜单、分组式设置及三档 HUD 配色。1.6.1 修复实验性 DDC 应答字段偏移、补完整 ad-hoc bundle 签名并同步文档。1.7.0 让设置窗口与菜单栏弹窗一致地磨砂玻璃化、略微放宽边缘入口条、新增莫兰迪 HUD 配色，并发布 Homebrew cask。1.7.1 修复磨砂窗口渲染、再次放宽入口条、提亮莫兰迪音量色。当前证据边界见 [BUILD_REPORT.md](BUILD_REPORT.md)。
