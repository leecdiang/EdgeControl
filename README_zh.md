...
手势识别、数值映射、档位、触觉反馈与设置层由 31 个单元测试覆盖。依赖未公开 macOS ABI 的代码全部通过 `dlopen`/`dlsym` 动态加载，符号缺失时优雅降级（该功能不可用，应用照常运行），而不是启动崩溃。
...
- 激活震动 + 2% 档位轻震（迟滞 + 频率限制）
- 可选的“仅下半部分”模式：边缘控制只响应触控板中线以下（中线 = 100%，底部 = 0%），中线以上落下的触点直接拒绝
...
默认情况下，触控板全高对应 0–100% 全范围。开启“仅下半部分”设置（设置 → System）后，边缘控制只响应触控板下半部分：中线对应 100%，最底部对应 0%。在中线以上落下的触点直接拒绝，适合上半部分容易碰到手掌或误触的情况。位置按触控板归一化，中线自动适配所有 Mac 机型。
