...
Touch contacts are processed in memory and reduced to identifier, normalized coordinates, timestamp, and phase. Release builds do not persist or transmit traces. Debug builds can print trace values to the process log under `EDGE_DEBUG_LOGGING` for local hardware tuning.
